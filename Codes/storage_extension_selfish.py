...
import sys
from showResult import restored_to_backed_up_ratio_plot, blocks_count_plot, selfish_node_count
...
class Node:
    ...
    def schedule_next_upload(self, sim: Backup):
        ...
        if self.is_selfish:
            #print(self.name)
            #print("Stopping program due to condition!")
            #sys.exit()
            return
    ...
    def schedule_next_download(self, sim: Backup):
        ...
        for block_id, (held_locally, peer) in enumerate(zip(self.local_blocks, self.backed_up_blocks)):
            if not held_locally and peer is not None and peer.online and peer.current_upload is None and not peer.is_selfish:   ####################
...
class Fail(Disconnection):
    ...
    def process(self, sim: Backup):
        ...
        total_lost = sum(sum(not block for block in node.local_blocks) for node in sim.nodes)  ########################### number of data loss
        sim.data_loss_count.append((total_lost, (sim.t / (365.25 * 24 * 60 * 60)))) ############################ save in data loss arrey
...
def main():
    ...
    parsing_functions = [
        ...
        ('arrival_time', parse_timespan), ('is_selfish', bool)
    ]
    ...
    #selfish_node_count(selfish_node_index1)
    selfish_node_count(selfish_node_index)
